...
# CP-even(odd) events if the rand is less than this probability 
	...
	print("C")
	df_sv=df_sv.insert(0,"wt",np.full((np.shape(df_sv)[0],1),weight),True)
	print("D")
	print(np.shape(df_sv))
	return  df_sv
...
ybcg=pd.concat(ylist)
y=pd.concat([ylist,y_sig])
y.columns=["class"]
...
for key, values in X.iteritems():
	print(key)
    	print("A")
    	print(values)
    	plot_signal_background(dflist[0], dfsignallist[0], key, bins=100)
...
X_train,X_test, y_train, y_test, wt_test  = train_test_split(
	X,
	y,
	test_size=0.2,
	random_state=123456,
...
model.fit(X_train, y_train,sample_weight=wt,batch_size=10000,epochs=100,callbacks=[history,early_stop],validation_data=(X_test, y_test))#, w_val))
